...
describe 'Equal Sides of an Array' do
  it 'should pass fixed tests' do
    ...
    expect(find_even_index([0,0,0,0,0])).to eq(0) #"Should pick the first index if more cases are valid"
    expect(find_even_index([-1,-2,-3,-4,-3,-2,-1])).to to(3)
    expect(find_even_index(Array(-100..-1))).to eq(-1)
  end

  it 'output should be an Integer' do
    expect(find_even_index(self).class).to eq(Integer)
  end
end
